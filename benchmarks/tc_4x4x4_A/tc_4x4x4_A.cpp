...
static const size_t rankB = 4;
static const size_t rankC = 4;

// Functions generated from TC
extern "C" {
void _mlir_ciface_tc4x4x4(memref::MemRefDescriptor<CIM_PRECISION, rankA> *A,
...
int main() {
  ...
  const int32_t E = DIM_SIZE;
  const int32_t F = DIM_SIZE;

  std::array<int64_t, rankA> aDim = {A, E, B, F};
  std::array<int64_t, rankB> bDim = {D, F, C, E};
  std::array<int64_t, rankC> cDim = {A, B, C, D};
  ...
  _mlir_ciface_tc4x4x4(&memA.memRefDesc, &memB.memRefDesc, &memC.memRefDesc);
  simulator_mark_end();
